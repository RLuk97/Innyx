...
// 3. Axios Interceptor (Garante que o token vá em cada busca)
    ...
    <div v-if="!token" class="relative flex min-h-screen items-center justify-center p-6 bg-[#050509]">
      ...
      <Transition name="fade" appear>
        <div class="relative w-full max-w-md bg-white/[0.03] backdrop-blur-3xl p-12 rounded-[3rem] border border-white/10 shadow-[0_50px_100px_-20px_rgba(124,58,237,0.15)] text-center">
          ...
          <img src="https://innyx.com/wp-content/uploads/2025/06/logo-innyx_branco.webp" class="h-12 mx-auto mb-10 opacity-100 drop-shadow-2xl">
          
          <div class="mb-8">
            <h2 class="text-[10px] font-black uppercase tracking-[0.5em] text-[#7c3aed]">Innyx Protocol</h2>
            <p class="text-slate-600 text-[9px] mt-1 font-bold italic uppercase tracking-widest text-center">Acesso ao Buffer de Gestão</p>
          </div>

          <form @submit.prevent="handleLogin" class="space-y-4 text-center">
            <input v-model="email" type="email" placeholder="E-mail Corporativo" 
              class="w-full bg-white/[0.05] border border-white/5 rounded-2xl px-6 py-4 text-white outline-none focus:border-[#7c3aed]/50 focus:bg-white/[0.08] transition-all placeholder:text-slate-600 text-sm">
            
            <input v-model="password" type="password" placeholder="Chave de Acesso" 
              class="w-full bg-white/[0.05] border border-white/5 rounded-2xl px-6 py-4 text-white outline-none focus:border-[#7c3aed]/50 focus:bg-white/[0.08] transition-all placeholder:text-slate-600 text-sm">

            <button :disabled="isLoading" class="w-full bg-white text-black font-black py-4 rounded-2xl hover:bg-[#7c3aed] hover:text-white transition-all shadow-xl shadow-white/5 uppercase tracking-[0.2em] text-[11px] mt-4">
    ...
    <div v-else class="animate-in fade-in duration-700">
          ...
          <div class="flex items-center gap-10">
            <div class="relative hidden lg:block">
              <input v-model="search" @input="fetchProducts(1)" type="text" placeholder="Filtrar base de dados..." 
                class="bg-white/5 border border-white/10 rounded-full px-12 py-2.5 text-xs focus:ring-1 focus:ring-[#7c3aed] outline-none w-72 text-white transition-all shadow-inner placeholder:text-slate-600">
              <span class="absolute left-4 top-3 opacity-20 text-[10px]">🔍</span>
            </div>
            <button @click="handleLogout" class="text-[9px] font-black uppercase tracking-[0.2em] text-red-500 hover:text-red-400 transition-colors">Sign Out</button>
          </div>
        </div>
      </header>
      ...
      <main class="pt-40 pb-20 px-8 max-w-7xl mx-auto">
        <div class="flex justify-between items-end mb-8">
            ...
            <button class="bg-[#7c3aed] text-white px-10 py-3 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-white hover:text-black transition-all shadow-xl shadow-[#7c3aed]/20">
                + Novo Registro
            </button>
        </div>
        ...
        <div class="bg-white/[0.02] backdrop-blur-3xl rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden relative">
          ...
          <table class="w-full border-collapse">
            <thead>
              <tr class="text-[9px] font-black uppercase tracking-[0.25em] text-slate-600 border-b border-white/5 bg-white/[0.01]">
                ...
                <th class="px-10 py-5 text-right">Interação</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-white/5">
              <tr v-for="product in products" :key="product.id" class="group hover:bg-white/[0.02] transition-colors duration-500">
                ...
                <td class="px-10 py-3.5">
                  <div class="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transform translate-x-2 group-hover:translate-x-0 transition-all duration-500">
                    <button class="px-4 py-1.5 rounded-xl bg-white/5 text-white text-[9px] font-black uppercase tracking-widest hover:bg-[#7c3aed] transition-colors border border-white/5">Editar</button>
                    <button class="w-8 h-8 rounded-xl bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all text-xs">🗑️</button>
                  </div>
                </td>
              </tr>
            </tbody>
          ...
          <footer class="p-8 flex justify-center items-center gap-12 border-t border-white/5">
            <button @click="fetchProducts(currentPage - 1)" :disabled="currentPage === 1" 
              class="w-10 h-10 rounded-xl border border-white/5 bg-white/5 flex items-center justify-center text-lg disabled:opacity-5 hover:bg-white hover:text-black transition-all shadow-sm text-white">←</button>
            
            <div class="flex flex-col items-center">
                <span class="text-[8px] font-black text-[#7c3aed] uppercase tracking-[0.4em] mb-1 italic">Buffer Page</span>
                <div class="text-xl font-black font-mono text-white tracking-tighter">
                  {{ currentPage }} <span class="text-white/10 mx-1">/</span> {{ lastPage }}
                </div>
            </div>
